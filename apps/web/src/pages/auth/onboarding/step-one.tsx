  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      <div className="space-y-1.5">
        ...
        <h2 className="text-2xl font-bold tracking-tight text-neutral-900">Let's setup your studio</h2>
        <p className="text-sm text-neutral-500">This is what clients will see. Make it yours.</p>
      </div>
      ...
      <div className="flex items-center gap-4 p-4 border border-dashed border-neutral-200 bg-neutral-50/50 rounded-xl cursor-pointer hover:bg-neutral-50 transition-all">
        ...
        <div>
          <p className="text-xs font-semibold text-neutral-700">Upload your logo</p>
          <p className="text-[10px] text-neutral-400">PNG or SVG · Max 2MB · Optional</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1.5">
          ...
          <Input id="studioName" placeholder="Alex Mercer Studio" {...register("studioName")} />
          {errors.studioName && <p className="text-xs text-rose-500">{errors.studioName.message as string}</p>}
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1.5">
          ...
          <Input id="professionalEmail" placeholder="alex@mercerstudio.com" {...register("professionalEmail")} />
          {errors.professionalEmail && <p className="text-xs text-rose-500">{errors.professionalEmail.message as string}</p>}
        </div>

        <div className="space-y-1.5">
          <Label htmlFor="currency">Your currency</Label>
          <select id="currency" {...register("currency")} className="w-full flex h-10 rounded-md border border-input bg-background px-3 py-2 text-sm">
